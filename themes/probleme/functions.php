 ...
 */
 function trimite_problema_create_user($username, $email) {
  ...
  $body = 'Salutare! Pentru a trimite o problemă este necesară crearea unui cont!
    Contul dumneavoastră a fost creat automat când ați trimis problema! Datele contului sunt numele: '.$username.' email: '. $email . ' și parola: ' . $password .'.
    Numai bine!';
...
function user_login_function() {
  ...
  $errors = [];
    ...
    if (isset($_POST['log-in_button']) && wp_verify_nonce($_POST['nonce'], 'userLogin')) {
      ...
      $user_email_or_user_name = $_POST['login_email'];
      $user_password = $_POST['login_password'];
      $looks_like_email = false;

      $check_if_username = username_exists($user_email_or_user_name);
      $check_if_email = email_exists($user_email_or_user_name);

      if(preg_match('(@)', $user_email_or_user_name)) {
            $looks_like_email = true;
    } 

      if ( empty($user_email_or_user_name) && !empty($user_password) ) {

          array_push($errors, __('Email-ul sau numele de utilizator este obligatoriu!'));

      } elseif(!empty($user_email_or_user_name) && empty($user_password)) {

          array_push($errors, __('Parola este obligatorie!'));

      } elseif (empty($user_email_or_user_name) && empty($user_password)) {

          array_push($errors, __('Email-ul sau numele de utilizator și parola sunt obligatorii!'));

      } elseif (!preg_match("/^[a-zA-Z-' ]*$/", $user_email_or_user_name) && !$looks_like_email) {

          array_push($errors, __('Numele de utlizator poate contine doar litere si spații!'));

      } elseif(!$check_if_username && !$looks_like_email  && !$check_if_email){

          array_push($errors, __('Nu există un cont cu acest nume de utilizator!'));

      }  elseif(!is_email($user_email_or_user_name ) && $looks_like_email) {

          array_push($errors, __('Adresa de email nu este validă!'));

      } elseif(is_email($user_email_or_user_name ) && !$check_if_email) {

          array_push($errors, __('Nu există un cont cu această adresă de email!'));

      } elseif(( $check_if_username && !empty($user_password)) || ( $check_if_email && !empty($user_password) )) {
        $credentials = array();
        $credentials['user_login'] =  $user_email_or_user_name;
...
function user_register() {
  if (isset($_POST['sign-up_button']) && wp_verify_nonce($_POST['nonce'], 'userRegister')) {
    ...
    global $errors;
      ...
      $user_first_name = trim($_POST['sign-up_first-name']);
      $user_last_name = trim($_POST['sign-up_last-name']);
      $user_email = trim($_POST['sign-up_email']);
      $user_phone = trim($_POST['sign-up_phone']);
      $user_password = trim($_POST['sign-up_password']);
      $user_confirm_password = trim($_POST['sign-up_confirm-password']);
    ...
    if(isset($_POST['problema_submit']))
    {
        ...
        if($validation['type'] == 'success' ) {
            ...
            if(!is_user_logged_in()) {
                ...
                $user_id = trimite_problema_create_user($user_name,  $user_mail);
